...
# expression
state: State = polymat.init_state()
...
x1: VariableExpression[State]
x2: VariableExpression[State]
x1, x2 = (polymat.define_variable(n) for n in ("x1", "x2"))

x: VariableVectorExpression[State] = polymat.v_stack((x1, x2))

# Create a polynomial expression using arithmetic operations
f: ScalarPolynomialExpression = (x1 + x2) + (x1 + x1 * x2)
...
sympy_repr: sympy.Expr
state, sympy_repr = polymat.to_sympy(
    f,
).apply(state)
...
print(
    f"{array_repr.data[2].toarray()=}"
)  # sparse scipy array converted to an numpy array
